...
def christofides(grafo):
    ...
    i = []
    for x in range(mst.number_of_nodes()):
        if mst.degree[x] % 2 != 0:
            i.append(x)
    aux = gf.edge_subgraph(gf.edges(i))
    m = nx.min_weight_matching(aux)
    g1 = nx.MultiGraph(mst)
    g1.add_edges_from(aux.edges(m))
    euleriano = list(nx.dfs_preorder_nodes(g1, source=0))
    return euleriano + [0]
...
# Euclideana
tsp = christofides(grafo.matrizEuc)
sum1 = 0
...
# Manhattan
tsp = christofides(grafo.matrizMhn)
sum2 = 0
for i in range(len(tsp) - 2):
